...
// 클래스 종류
// VO/DTO : ValueObject, Date Transfer Object
...
public class ClassMakeRule {
    public static void main(String[] args) {
        // 세금계산을 위한 객체 생성
        ComputeTax choi = new ComputeTax(  
                3500, 1);
        // << choi 객체 생성 ComputeTax 라는 선택자에 
        // int 변수 설정해둔 값을 넣고 출력할땐 생성선택자.만든 if문 변수를 넣고 해결
...
class ComputeTax {   // < VO 시작
    ...
    // 세금계산
    public double manageTax() {   //<< Service
        double tax = 0;
        if (isMarried == 0) { //기혼
            if (salary < 6000)
                    tax = salary * 0.15;
